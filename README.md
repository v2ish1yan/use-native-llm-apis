...
Some provider files are strong enough to drive end-to-end code generation. Some are only usable as starting notes. Some are still partial.
...
This repo currently has three active content tiers:
...
Older maintenance docs may still mention `skeleton` as a historical or reserved depth label, but the current live snapshot is tracked in [references/research/coverage-status.md](references/research/coverage-status.md).
